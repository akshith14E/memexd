...
final templateExampleProvider =
    StateNotifierProvider<TemplateExampleController, AsyncValue<List<Post>>>(
        (ref) => TemplateExampleController(ref.read));

class TemplateExampleController extends StateNotifier<AsyncValue<List<Post>>> {
  Reader _reader;
  List<Post>? get getPosts => state.data?.value;
  bool hasMorePosts = false;
  TemplateExampleController(this._reader) : super(AsyncData([]));
  void sortPostsbyTemplates(String url) async {
    ...
    state = AsyncLoading();
    final posts = await _fetchPostsbyTemplateId(url);
  ...
  _fetchPostsbyTemplateId(String url) async {
